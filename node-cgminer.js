	...
	try {
		socket = net.connect({ host: config.ip, port: config.port }, function () {
			...
			socket.on('end', function () {													// all data recieved from the response. Now pass to callback()
				...
				try { dataStg = JSON.parse(dataStg.replace('\x00', '')); }					// attempt to parse as an object, but if it fails, just return the string (my miners, for example, don't return proper json
